...
public class PathfindingDebugStepVisual : MonoBehaviour {
    ...
    [SerializeField] private Transform waterNode;
    ...
    public void Setup(Grid<PathNode> grid) {
        ...
        for (int x = 0; x < grid.GetWidth(); x++) {
            for (int y = 0; y < grid.GetHeight(); y++) {
                ...
                if (y == 5 || y == 4)
                    grid.GetGridObject(x, y).SetIsWater(true);
                Transform visualNode = CreateVisualNode(gridPosition, grid.GetGridObject(x, y).isWater);
    ...
    private Transform CreateVisualNode(Vector3 position,bool isWater) {
        Transform visualNodeTransform;
        if (!isWater)
             visualNodeTransform = Instantiate(pfPathfindingDebugStepVisualNode, position, Quaternion.identity);
        else
            visualNodeTransform = Instantiate(waterNode, position, Quaternion.identity);
